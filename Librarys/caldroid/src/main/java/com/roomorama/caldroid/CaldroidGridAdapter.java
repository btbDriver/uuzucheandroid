...
public class CaldroidGridAdapter extends BaseAdapter
{
    ...
    public static boolean chooseWorkDay = false;
    public static boolean chooseSat     = false;
    public static boolean chooseSun     = false;

    public static List<Integer> orderNoRentDaySecs = new ArrayList<Integer>();
    public static List<Integer> noRentDaySecs      = new ArrayList<Integer>();

    public static void setNoRentDaySecs(List<Integer> noRentDaySecs)
    {
        CaldroidGridAdapter.noRentDaySecs.clear();
        for (int v : noRentDaySecs)
        {
            CaldroidGridAdapter.noRentDaySecs.add(v);
        }
    }

    public static void setOrderNoRentDaySecs(List<Integer> orderNoRentDaySecs)
    {
        CaldroidGridAdapter.orderNoRentDaySecs.clear();
        for (int v : orderNoRentDaySecs)
        {
            CaldroidGridAdapter.orderNoRentDaySecs.add(v);
        }
    }

    // 0 星期天 1 星期一 2 星期二 3 星期三 4 星期四 5 星期五 6 星期六
    public static void setChooseWorkDay(boolean chooseWorkDay)
    {
        CaldroidGridAdapter.chooseWorkDay = chooseWorkDay;
        Date now = new Date();
        now.setMinutes(0);
        now.setHours(0);
        now.setSeconds(0);
        ...
        if (chooseWorkDay)
        {
            for (int start = 0; start <= 180; start++)
            {
                ...
                {
                    if (!noRentDaySecs.contains(nowTime + (start * 24 * 60 * 60)) && (!orderNoRentDaySecs.contains(nowTime + (start * 24 * 60 * 60))))
                    {
                        noRentDaySecs.add(nowTime + (start * 24 * 60 * 60));
                    }
                }
                startDay++;
                if (startDay == 7)
        ...
        else
        {
            for (int start = 0; start <= 180; start++)
            {
                if (startDay >= 1 && startDay <= 5)
                {
                    ...
                    {
                        noRentDaySecs.remove(new Integer(nowTime + (start * 24 * 60 * 60)));
                    }
                }
                startDay++;
                if (startDay == 7)
    ...
    public static void setChooseSat(boolean chooseSat)
    {
        CaldroidGridAdapter.chooseSat = chooseSat;
        Date now = new Date();
        now.setSeconds(0);
        now.setMinutes(0);
        now.setHours(0);
    ...
    public static void setChooseSun(boolean chooseSun)
    {
        CaldroidGridAdapter.chooseSun = chooseSun;
        Date now = new Date();
        now.setSeconds(0);
        now.setMinutes(0);
        now.setHours(0);
    ...
    public static void setOneDay(Date d)
    {
        int time = (int) (d.getTime() / 1000);
        if (!orderNoRentDaySecs.contains(time))
        {
            if (noRentDaySecs.contains(time))
            {
                noRentDaySecs.remove(new Integer(time));
            }
            else
            {
                noRentDaySecs.add(time);
            }
        }
    }
...
//        }
        ...
        if (textColorForDateTimeMap != null)
        {
            ...
            {
                if (textColorResource == 0xff000000)
                {
    ...
    {
        DateTime dt = datetimeList.get(position);
        return dt;
    }
...
//            }
        ...
        // day from now
        if (dayFromNowIn_180(dt))
        {


            if (orderNoRentDaySecs != null || noRentDaySecs != null)
            {
                if (orderNoRentDaySecs != null)
                {
                    int tar = (int) (dt.getMilliseconds(TimeZone.getTimeZone("GMT")) / 1000 - 8 * 60 * 60);
                    if (orderNoRentDaySecs.contains(tar))
                    {
                        cellView.setBackgroundResource(R.drawable.blue_calendar);
                        cellView.setTextColor(Color.WHITE);
                    }
                }
                if (noRentDaySecs != null)
                {
                    int tar = (int) (dt.getMilliseconds(TimeZone.getTimeZone("GMT")) / 1000 - 8 * 60 * 60);
                    if (noRentDaySecs.contains(tar))
                    {
                        cellView.setBackgroundResource(R.drawable.red_calendar);
                        cellView.setTextColor(Color.WHITE);
                    }
                }
            }
            ...
            {
                cellView.setBackgroundColor(0xffffffff);
                cellView.setTextColor(Color.BLACK);
            }
        }
        else
        {
            if (cellView.getCurrentTextColor() == Color.GREEN)
            {
                cellView.setTextColor(0x00ffffff);
                cellView.setBackgroundColor(0xffffffff);
            }
            else
            {
    ...
    private boolean dayFromNowIn_180(DateTime dt)
    {
        DateTime now = DateTime.today(TimeZone.getTimeZone("GMT"));
        int nDay = now.getDayOfYear();
        int nYear = now.getYear();
        int dDay = dt.getDayOfYear();
        int dYear = dt.getYear();
        if (dYear == nYear)
        {
            if (dDay - nDay <= 180 && dDay - nDay >= 0)
            {
                return true;
            }
            else
